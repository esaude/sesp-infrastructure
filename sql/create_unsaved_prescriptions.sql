...
FROM drug_order dord JOIN orders ord ON dord.order_id = ord.order_id AND dord.order_id NOT IN (SELECT drug_order_id FROM drug_order_relationship WHERE date_created > date('2019-10-01')) AND date_created > curdate() AND isnull(ord.date_stopped) AND  ISNULL(ord.previous_order_id);
